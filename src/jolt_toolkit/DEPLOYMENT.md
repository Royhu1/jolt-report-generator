...
- Python **≥ 3.10**, `pip install -r src/jolt_toolkit/requirements.txt` (the repo-level
  `requirements.txt` is a superset adding the pytest / black / isort / mypy toolchain).
- Not installable: no wheel, no console script. Vendor `src/jolt_toolkit/` as-is (the
  config JSONs travel inside it, so the copied folder is self-contained) and put `src/`
