...
module scope in the ROOT ``conftest.py`` (pytest imports it before it imports any
test module, and therefore before ``jolt_toolkit`` is first imported):
...
def frozen_config_data(fixtures_dir) -> dict:
    ...
    read the live ``src/jolt_toolkit/configs/*.json`` for a behavioural
